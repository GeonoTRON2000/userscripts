// ==UserScript==
// @name           TBG Blocks Plugin
// @namespace      GeonoTRON2000
// @description    Enables the TBG Blocks Plugin
// @version        1.0
...
TBGBlocks = {
	...
	styles: {
		...
		'turquoise':			'background-color: #62AFB0; border-color: #449192 #58A5A6 #58A5A6 #307D7E;',
	...
	blocksLib: {
		//BUMPING AND EDITING
		'bumpcurrenttopic': ['turquoise','stack'],
		'bumptopic': ['turquoise','stack'],
		'editpost': ['turquoise','stack'],

                //DELETING
		'deletepost': ['red','stack'],
		'deletetopic': ['red','stack'],

                //POSTING
		'posttopiconforumwithtitleandtext': ['blue','stack'],
		'replytotopicwithtext': ['blue','stack'],
                'lastposterontopic': ['blue','reporter'],
                'topicisclosed': ['blue','boolean'],

                //SMILEYS AND BBCODE
                'parsesmileys': ['yellow','reporter'],
                'parsebbcode': ['yellow', 'reporter'],
...
for (i = 0; i < apply.length; i++) {
  var thishtml = apply[i].innerHTML;
  var j;
  var scripts = [];
  for (j = 0; j < thishtml.split("[tbgblocks]").length-1; j++) {
    scripts[j] = thishtml.split("[tbgblocks]")[j+1].split("[/tbgblocks]")[0];
  }
  for (j = 0; j < scripts.length; j++) {
    thishtml = thishtml.replace(/\[tbgblocks\](.+?)\[\/tbgblocks\]/i, "<pre class=\"tbgblocks\">"+scripts[j].replace(/<br>/gi, "\n").replace(/<br \/>/gi, "\n")+"</pre>");
  }
  apply[i].innerHTML = thishtml;
}

TBGBlocks.parse({containerTag: 'pre', containerClass: 'tbgblocks'});